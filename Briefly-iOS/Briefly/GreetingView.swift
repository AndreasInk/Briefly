...
struct GreetingView: View {
    ...
    var body: some View {
        ZStack {
            GradientBackgroundView(colors: GradientBackgroundView.greenGradientColors)
                .scaleEffect(3, anchor: .top)
            VStack {
                
                if isVisible {
                    let visualEffects = Text("your briefing is ready...")
                        .customAttribute(EmphasisAttribute())
                        .foregroundStyle(.thickMaterial)
                        .bold()
                    
                    Text("Good morning, \(visualEffects)")
...
struct AppearanceEffectRenderer: TextRenderer, Animatable {
    ...
    func draw(_ slice: Text.Layout.RunSlice, at time: TimeInterval, in context: inout GraphicsContext) {
        // Calculate a progress value in unit space for blur, glow, and
        ...
        // Make the glow appear quickly and then fade out
        let glowProgress = min(progress * 2, 1.0)
        let glowRadius =
            slice.typographicBounds.rect.height / 8 *
            UnitCurve.easeIn.value(at: glowProgress)
        ...
        if glowProgress < 1.0 {
            context.addFilter(.shadow(color: Color.teal.opacity(0.6 * (1 - glowProgress)), radius: glowRadius))
        }
